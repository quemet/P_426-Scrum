...
cafesRouter.get("/", auth, (req, res) => {
    if(req.query.name) {
        ...
        return Cafe.findAll({ where: { name: { [Op.like]: `%${req.query.name}%` } }, order: ["Nom_Coffee"], limit: limit,
            ...
            const message = `Il y a ${cafes.count} cafés qui correspondent au terme de la recherche`;
...
cafesRouter.get("/:id", auth, (req, res) => {
    Cafe.findByPk(req.params.id).then((cafe) => {
        ...
        const message = `Le café dont l'id vaut ${cafe.id} a bien été récupéré.`;
...
cafesRouter.delete("/:id", auth, (req, res) => {
    Cafe.findByPk(req.params.id).then((deletedCafe) => {
        ...
        return Cafe.destroy({ where: { id: deletedCafe.id }, }).then((_) => {
            const message = `Le café ${deletedCafe.name} a bien été supprimé !`;
...
cafesRouter.put("/:id", auth, (req, res) => {
    ...
    Cafe.update(req.body, { where: { id: cafeId } }).then((_) => {
        return Cafe.findByPk(cafeId).then((updatedCafe) => {
            if(updatedCafe === null) {
                const message = "Le café demandé n'existe pas. Merci de réessayer avec un autre identifiant.";
                return res.status(404).json({ message });
            }
            const message = `Le cafe ${updatedCafe.name} dont l'id vaut ${cafeId} a été mis à jour avec succès !`;
